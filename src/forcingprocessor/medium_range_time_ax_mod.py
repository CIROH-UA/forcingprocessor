...
This script reads in a NRDS forcing file and shifts the time coordinate for
medium range ensemble members
...
def cut_forcing_data_for_ensemble(
    ds: xr.Dataset, ens_member_arg: int, time_shift_hours_arg: int = 6
) -> xr.Dataset:
    """
    ...
    Returns:
    xarray.Dataset: Dataset with shifted time axis.
    """
    start_cut = (ens_member_arg - 1) * time_shift_hours_arg
    end_cut = 204 + start_cut
    print(
        f"Member {ens_member_arg}, cutting data from time axis index {start_cut} to {end_cut}"
    )
...
if __name__ == "__main__":
    ...
    variables = list(ds_mod.keys())
    variables.remove("Time")
    variables.remove("ids")
    for j, jvar in enumerate(variables):
