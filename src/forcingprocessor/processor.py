...
from forcingprocessor.utils import make_forcing_netcdf, get_window, log_time, convert_url2key
from forcingprocessor.utils import report_usage, nwm_cfe_variables, ngen_cfe_variables
...
def write_netcdf(data:np.ndarray, t_ax:list, catchments:list, prefix:str, filename:str,
    ...
    if storage_type == 's3':
        ...
        with tempfile.NamedTemporaryFile(suffix='.nc') as tmpfile:
            make_forcing_netcdf(tmpfile.name, catchments, t_utc, data, ngen_variables)
        ...
        make_forcing_netcdf(nc_filename, catchments, t_utc, data, ngen_variables)
...
def multiprocess_write_netcdf(data:np.ndarray, jcatchment_dict:dict, t_ax:np.ndarray,
    ...
    with cf.ProcessPoolExecutor(max_workers=min(njobs,nprocs)) as pool:
        for results in pool.map(
            ...
            filenames,
            [storage_type for x in range(njobs)],
            [ngen_variables for x in range(njobs)]
            ):
...
def prep_ngen_data(conf):
    ...
    data_array, t_ax, nwm_data, nwm_file_sizes_MB = multiprocess_data_extract(nwm_forcing_files,
                                                                              nprocs,weights_df,fs,
                                                                              ngen_variables,
                                                                              nwm_variables)
    ...
    if ii_plot or ii_collect_stats or any(x in output_file_type for x in ["csv","parquet","tar"]):
        forcing_cat_ids, filenames, individual_cat_file_sizes_MB, individual_cat_file_sizes_MB_zipped, tar_buffs = multiprocess_write_df(
            data_array,t_ax,list(weights_df.index),nprocs,forcing_path,ngen_variables_to_print)
