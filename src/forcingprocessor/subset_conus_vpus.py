    ...
    for j, jvpu in enumerate(VPUs):
        ...
        for jlayer in layer_names:
            if jlayer == "divides" or jlayer == "divide-attributes":
                ...
                data_jvpu_jlayer = gpd.GeoDataFrame(
                    data_conus_jlayer[data_conus_jlayer["vpuid"] == jvpu]
                )
                if jlayer == "divides":
                    ...
                    attrs_df = gpd.GeoDataFrame(
                        attrs_df.drop(
                            attrs_df.index[
                                list(
                                    np.where(
                                        ["+" in x for x in list(attrs_df["divide_id"])]
                                    )[0]
                                )
                            ]
                        )
                    ...
                    if nattrs != ncats:
                        ...
                        for jcat_in_attr in cats_in_attrs:
                            if jcat_in_attr not in cats_in_divides:
                                attrs_df = gpd.GeoDataFrame(
                                    attrs_df[attrs_df["divide_id"] != jcat_in_attr]
                                )
                                divides_df = gpd.GeoDataFrame(
                                    divides_df[divides_df["divide_id"] != jcat_in_attr]
                                )
                        ...
                        for jcat_in_divides in cats_in_divides:
                            if jcat_in_divides not in cats_in_attrs:
                                attrs_df = gpd.GeoDataFrame(
                                    attrs_df[attrs_df["divide_id"] != jcat_in_divides]
                                )
                                divides_df = gpd.GeoDataFrame(
                                    divides_df[
                                        divides_df["divide_id"] != jcat_in_divides
                                    ]
                                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--raster_file", help="Path to example nwm CONUS forcing file", default=""
    )
