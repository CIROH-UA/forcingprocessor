...
def log_time(label: str, log_file: str) -> None:
    """Write timestamp for each process step into log file.
...
def report_usage() -> tuple[float, float, float]:
    """Report current system usage.

    Returns:
        tuple[float, float, float]: RAM usage (GB), RAM percentage, and CPU percentage.
    """
    ...
    print(
        ...
        + f"Current CPU usage : {percent_cpu:.2f}%"
    )
    return usage_ram, percent_ram, percent_cpu


def convert_url2key(nwm_file: str | Path, fs_type: str | None) -> tuple[str, str]:
    """Convert a URL to a bucket and key.

    Args:
        nwm_file (str | Path): The URL or path of the NWM file
        fs_type (str): The file system type ("google" or "s3")

    Returns:
        tuple[str, str]: The bucket and key for the file
    """
    bucket_key = ""
...
def make_forcing_netcdf(
    out_path: Path | str, catchments: np.ndarray, t_ax: np.ndarray, input_array: np.ndarray
) -> None:
    """
    Create a netcdf file with the forcing data.

    Parameters:
    out_path (Path | str): Path to save the netcdf file.
