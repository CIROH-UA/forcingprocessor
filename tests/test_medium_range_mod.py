...
def test_cut_ens_data():
    ...
    data["ids"] = (
        ("catchment-ids",),
        np.array([ncatember for ncatember in range(ncat)]),
    )
    data["Time"] = (
        ("catchment-ids", "time"),
        np.tile(
    ...
    for ens_member in range(2, 8):
        ...
        assert ds_cut.UGRD_10maboveground.shape[1] == 204, (
            "Output data does not have 204 time steps"
        )
        assert ds_cut.Time.shape[1] == 204, (
            "Output time axis does not have 204 time steps"
        )
        assert (
            ds_cut.Time.values[0, 0] == ds.Time.values[0, member_shifts[ens_member][0]]
        ), "Output time axis start does not match expected value"
