...
conf = {
    "forcing": {
        ...
        "gpkg_file": [
            f"{pwd}/docs/examples/routing-only_example/cat-1555522_subset.gpkg"
        ],
...
def test_nomads_prod(download_weight_file, clean_forcings_metadata_dirs):
    ...
    assert_file = (
        data_dir / "outputs/ngen/ngen.t00z.short_range.channel_routing.f001_f001.nc"
    ).resolve()
...
def test_noaa_nwm_pds_https_medium_range(
    ...
    assert_file = (
        data_dir
        / "outputs/ngen/ngen.t00z.medium_range_mem1.channel_routing.f001_f001.nc"
    ).resolve()
    assert assert_file.exists()
    os.remove(assert_file)
...
def test_noaa_nwm_pds_https_analysis_assim(
    ...
    prep_ngen_data(conf)
    assert_file = (
        data_dir / "outputs/ngen/ngen.t00z.analysis_assim.channel_routing.tm01_tm01.nc"
    ).resolve()
    assert assert_file.exists()
    os.remove(assert_file)
...
def test_noaa_nwm_pds_https_analysis_assim_extend(
    ...
    assert_file = (
        data_dir
        / "outputs/ngen/ngen.t16z.analysis_assim_extend.channel_routing.tm01_tm01.nc"
    ).resolve()
    assert assert_file.exists()
    os.remove(assert_file)
